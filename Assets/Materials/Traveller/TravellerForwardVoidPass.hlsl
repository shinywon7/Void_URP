...
Light GetVoidLight(float3 normal)
{
    Light light;
    light.direction = normal;
    light.color = float4(0.3,0.3,0.3,1);
...
half4 VoidFragmentPBR(InputData inputData, SurfaceData surfaceData, float3 normal)
{
    ...
    Light mainLight = GetVoidLight(normal);
...
)
{
    ...
    if(vPos1.z > height1 || vPos2.z-_VoidWidth > height2) {
        outColor = 0;
        return;
    }
    ...
    half4 color = VoidFragmentPBR(inputData, surfaceData, input.normalWS);
