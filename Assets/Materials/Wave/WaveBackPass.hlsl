...
Varyings LitPassVertex(Attributes input)
{
    ...
    if(vertexIndex < _Resolution*_Resolution){
        float4 height = LOAD_TEXTURE2D_X_LOD(_BackHeightTexture, int2(vertexIndex/_Resolution, vertexIndex%_Resolution),0);
...
half4 CalcBackgroundColor(Varyings input, float3 viewDirWS, float facing, float refractionIndex){
    ...
    for(int j = 0; j <10;j++){
        ...
        float3 targetVS = TransformWorldToView(target);
        ...
        float targetDepth = LinearEyeDepth(depth.r, _ZBufferParams);
    ...
    float2 uv = getCS2UV(TransformWorldToHClip(surfaceWS));
    

    return SAMPLE_TEXTURE2D_X(_BackSideTexture, sampler_BackSideTexture, targetUV);
}
...
)
{
    ...
    InitializeStandardLitSurfaceData(input.uv, surfaceData, 0);
    ...
    surfaceData.emission =  CalcBackgroundColor(input, viewDirWS, facing, refractionIndex) * (1-_BaseColor.a);
