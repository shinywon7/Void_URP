Shader "Hidden/Void/Blit/BackHeightSimulate"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D_X_FLOAT(_BackRelativeVelocityTexture);
            SAMPLER(sampler_BackRelativeVelocityTexture);
            ...
            float3 getRelatveVelocity(float2 uv){
                ...
                return SAMPLE_TEXTURE2D_X(_BackRelativeVelocityTexture, sampler_BackRelativeVelocityTexture, uv).xyz + float3(wind*0.1,0,wind);
            }
        ...
        Pass
        {
            ...
            float3 getRelatveVelocity(float2 uv){
                return SAMPLE_TEXTURE2D_X(_BackRelativeVelocityTexture, sampler_BackRelativeVelocityTexture, uv).xyz;
            }
            #include "HeightSimulate.hlsl"
        ...
        Pass
        {
            ...
            #pragma vertex Vert
            #pragma fragment frag
            TEXTURE2D_X_FLOAT(_BackHeightTexture);
            SAMPLER(sampler_BackHeightTexture);
            
            float4 frag (Varyings input) : SV_Target
            {
                //input.texcoord.y=1-input.texcoord.y;
                float4 Color = SAMPLE_TEXTURE2D_X(_BackHeightTexture, sampler_BackHeightTexture, input.texcoord);
				return Color;
            }
            ENDHLSL
        }
